...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  if (book) {
    ...
    if (reviews) {
      book["reviews"] = reviews; // Update reviews if provided
    }
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (isbn) {
    delete books[isbn];
    return res.status(200).send(`Book with the ISBN ${isbn} deleted.`); // Respond with success message
  }
  else {
  return res.status(404).json({ message: `Book with the ISBN ${isbn} doesn't exist.`}); // Respond with message
  }
});
